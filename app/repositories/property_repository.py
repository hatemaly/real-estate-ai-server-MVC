...
from app.models.property_models.property import (
    Property,
    BackOfficePropertyDetails,
    PropertyType,
)
...
class PropertyRepository(BaseRepository):
    ...
    async def update_price(
        self, property_id: str, price_index: int, price: dict
    ) -> UpdateResult:
        return await self.collection.update_one(
            {"_id": ObjectId(property_id)},
            {
                "$set": {
                    f"background_property_details.price_history.{price_index}": price
                }
            },
        )
    ...
    async def activate_price(self, property_id: str, price_index: int) -> UpdateResult:
        return await self.collection.update_one(
            ...
            {
                "$set": {
                    f"background_property_details.price_history.{price_index}.is_active": True
                }
            },
        )

    async def deactivate_price(
        self, property_id: str, price_index: int
    ) -> UpdateResult:
        return await self.collection.update_one(
            {"_id": ObjectId(property_id)},
            {
                "$set": {
                    f"background_property_details.price_history.{price_index}.is_active": False
                }
            },
        )

    # Payment Plan Management
    async def add_price_payment_plan(
        self, property_id: str, price_index: int, payment_plan: dict
    ) -> UpdateResult:
        return await self.collection.update_one(
            {"_id": ObjectId(property_id)},
            {
                "$push": {
                    f"background_property_details.price_history.{price_index}.payment_plans": payment_plan
                },
                "$inc": {
                    f"background_property_details.price_history.{price_index}.payment_plans_count": 1
                },
            },
        )

    async def update_price_payment_plan(
        self, property_id: str, price_index: int, plan_index: int, payment_plan: dict
    ) -> UpdateResult:
        return await self.collection.update_one(
            {"_id": ObjectId(property_id)},
            {
                "$set": {
                    f"background_property_details.price_history.{price_index}.payment_plans.{plan_index}": payment_plan
                }
            },
        )

    async def activate_price_payment_plan(
        self, property_id: str, price_index: int, plan_index: int
    ) -> UpdateResult:
        return await self.collection.update_one(
            {"_id": ObjectId(property_id)},
            {
                "$set": {
                    f"background_property_details.price_history.{price_index}.payment_plans.{plan_index}.is_active": True
                }
            },
        )

    async def deactivate_price_payment_plan(
        self, property_id: str, price_index: int, plan_index: int
    ) -> UpdateResult:
        return await self.collection.update_one(
            {"_id": ObjectId(property_id)},
            {
                "$set": {
                    f"background_property_details.price_history.{price_index}.payment_plans.{plan_index}.is_active": False
                }
            },
        )
    ...
    async def set_highlighted_payment_plan(
        self, property_id: str, price_index: int, plan_index: int
    ) -> UpdateResult:
        return await self.collection.update_one(
            {"_id": ObjectId(property_id)},
            {
                "$set": {
                    f"background_property_details.price_history.{price_index}.highlighted_payment_plan": plan_index
                }
            },
        )
    ...
    async def update_basic_info(
        self, property_id: str, basic_info: dict
    ) -> UpdateResult:
        return await self.collection.update_one(
            {"_id": ObjectId(property_id)},
    ...
    async def update_detailed_info(
        self, property_id: str, detailed_info: dict
    ) -> UpdateResult:
        return await self.collection.update_one(
            {"_id": ObjectId(property_id)},
    ...
    async def update_background_info(
        self, property_id: str, background_info: dict
    ) -> UpdateResult:
        return await self.collection.update_one(
            {"_id": ObjectId(property_id)},
    ...
    async def get_properties_by_status(self, is_active: bool) -> List[dict]:
        return await self.collection.find(
            {"background_property_details.is_active": is_active}
        ).to_list(None)
    ...
    async def get_properties_by_criteria(
        ...
        if locations_ids:
            filter_query["detailed_property_details.location_ids"] = {
                "$in": [ObjectId(lid) for lid in locations_ids]
            }

        if developers_ids:
            filter_query["basic_property_details.developer_ids"] = {
                "$in": [ObjectId(did) for did in developers_ids]
            }

        if projects_ids:
            filter_query["basic_property_details.parent_project_id"] = {
                "$in": [ObjectId(pid) for pid in projects_ids]
            }
        ...
        async for doc in cursor:
            ...
            if (
                "basic_property_details" in doc
                and "developer_ids" in doc["basic_property_details"]
            ):
                doc["basic_property_details"]["developer_ids"] = [
                    str(did) for did in doc["basic_property_details"]["developer_ids"]
                ]
            if (
                "detailed_property_details" in doc
                and "location_ids" in doc["detailed_property_details"]
            ):
